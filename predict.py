...
def main(model_name, data_path, n2vname):
    ...
    if torch.cuda.is_available():
        model.cuda()
    val_features, val_graphs, val_labels = load_data(data_path + testset, data_path + testlabel, Dataset_Path + "pssm/", Dataset_Path + "graph/")
    mcc, fsc, prec, recall, acc, sensi, speci, auc = evaluate(model,val_features, val_graphs, val_labels, model_name, data_path)
